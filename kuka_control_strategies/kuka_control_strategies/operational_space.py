...
from kuka_control_strategies.computed_torque import (
    dh_matrix,
...
class OperationalSpaceController(Node):
    ...
    def __init__(self):
        ...
        if self.target_name == "custom":
            ...
            self.p_des = CARTESIAN_TARGETS.get(
                self.target_name, CARTESIAN_TARGETS["center"]
            )
            self.q_seed = np.array(
                JOINT_SEEDS.get(self.target_name, JOINT_SEEDS["center"])
            )
